...
pub fn query_replace_body(
    ...
    no_format: bool,
    dry_run: bool,
) -> Result<crate::lsp::refactor::EditOutput> {
    let path = resolve_file(file)?;
    let project_root = find_root(&path)?;
    crate::lsp::refactor::replace_body(
        &path,
        name,
        class,
        content,
...
pub fn query_replace_symbol(
    ...
    let path = resolve_file(file)?;
    let project_root = find_root(&path)?;
    crate::lsp::refactor::replace_symbol(
        &path,
        name,
        class,
        content,
