        ...
        && let Some(func_node) = node.child_by_field_name("function")
            && func_node.kind() == "attribute" {
                ...
                if let Some(method_node) = last_child {
                    ...
                    if method_text.starts_with('_') && object_text != "self" && !ALLOWED_CALLBACKS.contains(&method_text.as_str()) {
                        diags.push(LintDiagnostic {
                            rule: "private-method-access",
                            message: format!("accessing private method `{}` on external object", method_text),
