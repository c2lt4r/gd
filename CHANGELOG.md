...
### Changed
- **Rename, change-signature, extract-method now use typed AST rewriter** — these commands no longer splice strings at byte offsets. They parse to the owned typed AST, apply rewrite rules (`rewrite_file` for expressions, direct `Vec` mutation for declarations/parameters), and print back via the span-preserving printer. Unchanged code emits original source bytes verbatim.
...
- **Regenerate ClassDB from runtime-verified Godot metadata** — richer generated data: 771 operator type combinations, 154 builtin type constructors, 128 function return types, 36 annotation definitions, 48 warning codes, type conversion matrices. Generator now filters out 469 void "properties" that were inspector category headers, not real properties (one was shadowing the Input engine class in `lookup_member`).
...
- **Wire generated operator table into type inference** — `infer_binary` now queries 771 ClassDB operator combinations before falling back to heuristics. Covers all Godot builtin type operator results (e.g. `Basis * Vector3 → Vector3`, `AABB * Transform3D → AABB`).
- **Replace hand-written engine class member docs with ClassDB** — removed 1000+ lines of manually maintained hover/completion data for 14 engine classes (Object, Node, Node2D, etc.). `members_for_class`, `lookup_member`, `lookup_member_for` now query ClassDB's 16k+ methods and 5.7k+ properties dynamically. Builtin variant type members (Vector2, String, etc.) remain in the generated static table.
- **Replace hardcoded virtual method lists with generated table** — generator now extracts 1147 virtual method names from ClassDB into `VIRTUAL_METHODS`. `is_godot_virtual_method()` uses binary search on this table. Replaced 4 hand-maintained lists: `GODOT_VIRTUALS` (42 entries), `LIFECYCLE_METHODS` (18 entries), `GODOT_BUILTINS` (14 entries), and `LIFECYCLE_METHOD_DOCS` (11 entries). Lifecycle method hover docs now come from ClassDB.
...
- **Centralize curated domain knowledge** — new `gd-class-db::curated` module. Moved 5 scattered const arrays from lint rules into one place. Area monitoring signals and print functions are now derived from ClassDB (validated in tests). Semantic constraints (tree-dependent methods, physics-only methods) remain hand-written — ClassDB knows these APIs exist but doesn't encode their usage constraints.
