...
impl WorkspaceIndex {
    ...
    fn scan(&self) {
        if let Ok(entries) = gd_core::fs::collect_gdscript_files(&self.project_root) {
            for path in entries {
        ...
        if let Ok(scene_files) = gd_core::fs::collect_resource_files(&self.project_root) {
    ...
    fn index_scene(&self, path: &Path, content: &str) {
        let Ok(scene_data) = gd_core::scene::parse_scene(content) else {
            return;
        };
...
fn root_from_path(path: &Path) -> Option<PathBuf> {
    gd_core::project::GodotProject::discover(path)
        .ok()
        .map(|p| p.root)
}
