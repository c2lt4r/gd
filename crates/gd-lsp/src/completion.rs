...
pub(super) fn resolve_simple_receiver(
    ...
    if receiver == "self" || receiver == "super" {
        if let Ok(tree) = gd_core::parser::parse(source) {
            ...
            return match file.extends {
                Some(gd_core::gd_ast::GdExtends::Class(name))
    ...
    if gd_class_db::class_exists(receiver) {
    ...
    if let Ok(tree) = gd_core::parser::parse(source) {
        ...
        let file = gd_core::gd_ast::convert(&tree, source);
        if let Some(gd_core::gd_ast::GdExtends::Class(extends_class)) = file.extends
            && gd_class_db::class_exists(extends_class)
...
fn resolve_chain(
    ...
    if let Some(ws) = workspace {
            ...
            && let Ok(tree) = gd_core::parser::parse(&content)
        {
            ...
            if gd_file
                .declarations
                .iter()
                .any(|d| matches!(d, gd_core::gd_ast::GdDecl::Enum(e) if e.name == member))
            {
            ...
            if gd_file
                .declarations
                .iter()
                .any(|d| matches!(d, gd_core::gd_ast::GdDecl::Signal(s) if s.name == member))
            {
    ...
    if (head == "self" || head == "super")
        && let Ok(tree) = gd_core::parser::parse(source)
        && {
...
pub(super) fn resolve_member_type(class: &str, member: &str) -> Option<String> {
    ...
    for (name, prop_type, _) in gd_class_db::class_properties(class) {
        if name == member {
            return Some(normalize_type(prop_type));
        }
    }
    // Check ClassDB method return types
    if let Some(ret) = gd_class_db::method_return_type(class, member)
        && ret != "void"
    {
...
fn provide_class_dot_completions(
    ...
    let db_class = if gd_class_db::class_exists(class_name)
        || !super::builtins::members_for_class(class_name).is_empty()
    {
        ...
        content.and_then(|c| {
            ...
            let file = gd_core::gd_ast::convert(&tree, &c);
            match file.extends {
                Some(gd_core::gd_ast::GdExtends::Class(name))
                    if gd_class_db::class_exists(name) =>
                {
...
fn collect_enum_dot_completions(
    ...
    let Ok(tree) = gd_core::parser::parse(file_content) else {
        return Vec::new();
    };
    let gd_file = gd_core::gd_ast::convert(&tree, file_content);

    for decl in &gd_file.declarations {
        if let gd_core::gd_ast::GdDecl::Enum(e) = decl
            && e.name == enum_name
        {
            let mut items = Vec::new();
...
fn collect_class_db_dot_items(
    ...
    for (method_name, ret_type, owner_class) in gd_class_db::class_methods(class_name) {
    ...
    for (prop_name, prop_type, owner_class) in gd_class_db::class_properties(class_name) {
...
fn member_doc(class: &str, name: &str) -> Option<Documentation> {
    ...
    if let Some(doc) = gd_class_db::method_doc(class, name) {
        return Some(Documentation::MarkupContent(MarkupContent {
            kind: MarkupKind::Markdown,
            value: doc.to_string(),
        }));
    ...
    if let Some(doc) = gd_class_db::property_doc(class, name) {
        return Some(Documentation::MarkupContent(MarkupContent {
            kind: MarkupKind::Markdown,
            value: doc.to_string(),
        }));
    ...
    if let Some(ev) = gd_class_db::enum_value_doc(class, name)
        && !ev.doc.is_empty()
    {
        return Some(Documentation::MarkupContent(MarkupContent {
            kind: MarkupKind::Markdown,
        ...
        if child.kind() == "extends_statement" {
            ...
            for c in child.children(&mut inner) {
                if c.kind() != "extends" {
                    ...
                    if gd_class_db::class_exists(text) {
...
fn collect_class_db_methods(class: &str, items: &mut Vec<CompletionItem>) {
    for (method_name, ret_type, owner_class) in gd_class_db::class_methods(class) {
        let documentation = member_doc(owner_class, method_name);
        items.push(CompletionItem {
            label: method_name.to_string(),
            kind: Some(CompletionItemKind::METHOD),
