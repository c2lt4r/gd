...
pub fn inline_method(
    ...
    if let Some(ref target_file) = call_info.target_file {
        ...
        def_tree = gd_core::parser::parse(&def_source)?;
        ...
        cross_file_relative = Some(gd_core::fs::relative_slash(target_file, project_root));
    } else {
        ...
        if let Some(local_def) = same_file_def {
            ...
            cross_file_relative = Some(gd_core::fs::relative_slash(&cross.path, project_root));
    ...
    let can_delete = if is_cross_file {
        ...
        let all_refs =
            crate::references::find_references_by_name(func_name, &workspace, None, None);
...
fn inline_by_name_same_file(
    ...
    if dry_run {
        ...
        if all && inlined_count == sites_to_inline.len() as u32 {
            ...
            let current_tree = gd_core::parser::parse(&current_source)?;
    ...
    match matches.len() {
        ...
        n => {
            let file_list: Vec<String> = matches
                .iter()
                .map(|p| gd_core::fs::relative_slash(p, project_root))
                .collect();
            Err(miette::miette!(
