...
use gd_core::gd_ast::{self, GdDecl, GdExtends, GdFile};
...
fn resolve_hover_for_identifier(
    ...
    if let Some(ev) = gd_class_db::enum_value_doc("@GlobalScope", name) {
    ...
    if gd_class_db::class_exists(name) {
        ...
        let doc = gd_class_db::class_doc(name);
    ...
    if let Some(ws) = workspace
        ...
        && let Ok(tree) = gd_core::parser::parse(&content)
    {
...
fn hover_member_on_type(
    ...
    let db_class = if gd_class_db::class_exists(class) {
        ...
        let content = ws
            ...
            .or_else(|| ws.autoload_content(class));
        content.and_then(|c| {
            let tree = gd_core::parser::parse(&c).ok()?;
            let f = gd_ast::convert(&tree, &c);
            match f.extends {
                Some(GdExtends::Class(name)) if gd_class_db::class_exists(name) => {
                    Some(name.to_string())
                }
                _ => None,
            }
    ...
    // 2. ClassDB property
    for (name, prop_type, owner_class) in gd_class_db::class_properties(db_class) {
        if name == member {
            let code = format!("{prop_type} {owner_class}.{name}");
            let doc = gd_class_db::property_doc(db_class, name);
            return Some(make_hover(&code, ident_node, doc));
        }
    }

    // 3. ClassDB method
    if let Some(ret) = gd_class_db::method_return_type(db_class, member) {
        let owner = find_method_owner(db_class, member).unwrap_or(db_class);
        let code = format!("{owner}.{member}() -> {ret}");
        let doc = gd_class_db::method_doc(db_class, member);
        return Some(make_hover(&code, ident_node, doc));
    }
    ...
    loop {
        ...
        match gd_class_db::parent_class(cur) {
    ...
    if let Some(ev) = gd_class_db::enum_value_doc(db_class, member) {
        return Some(make_hover(
            &format_enum_value(ev),
            ident_node,
            if ev.doc.is_empty() {
    ...
    if gd_class_db::enum_type_exists(db_class, member) {
        let values = gd_class_db::enum_values(db_class, member);
        if !values.is_empty() {
...
fn find_method_owner<'a>(class: &str, method: &str) -> Option<&'a str> {
    for (name, _, owner) in gd_class_db::class_methods(class) {
        if name == method {
            return Some(owner);
        }
    }
...
fn resolve_classdb_member_for_file(
    ...
    // Try property
    for (prop_name, prop_type, owner_class) in gd_class_db::class_properties(extends) {
        if prop_name == name {
            let code = format!("{prop_type} {owner_class}.{prop_name}");
            let doc = gd_class_db::property_doc(extends, prop_name);
            return Some(make_hover(&code, ident_node, doc));
        }
    }
    // Try method
    if let Some(ret) = gd_class_db::method_return_type(extends, name) {
        let owner = find_method_owner(extends, name).unwrap_or(extends);
        let code = format!("{owner}.{name}() -> {ret}");
        let doc = gd_class_db::method_doc(extends, name);
        return Some(make_hover(&code, ident_node, doc));
    }
    None
}
...
fn resolve_builtin_member_for_file<'a>(file: &GdFile, name: &str) -> Option<&'a BuiltinMember> {
    ...
    loop {
        ...
        match gd_class_db::parent_class(current) {
...
fn extends_classdb_class<'a>(file: &GdFile<'a>) -> Option<&'a str> {
    match file.extends {
        Some(GdExtends::Class(name)) if gd_class_db::class_exists(name) => Some(name),
        _ => None,
    }
}

/// Format a single enum value for hover display.
fn format_enum_value(ev: &gd_class_db::EnumValue) -> String {
    format!("{}.{} = {}", ev.enum_name, ev.name, ev.value)
}

/// Format a listing of all enum values for hover display.
fn format_enum_listing(values: &[gd_class_db::EnumValue]) -> String {
    let mut lines = Vec::new();
    for v in values {
        if v.doc.is_empty() {
