...
pub fn format_document(source: &str, options: &FormattingOptions) -> Option<Vec<TextEdit>> {
    ...
    let mut config = gd_core::config::FmtConfig::default();
    ...
    let mut printer = gd_fmt::printer::Printer::from_config(&config);
    ...
    if gd_fmt::verify_format(source, &formatted, &config).is_some() {
        return Some(vec![]);
    }
