...
fn resolve_method_signature(
    ...
    } else if gd_class_db::class_exists(receiver) {
    ...
    if let Some(ws) = workspace {
        ...
        if let Some(content) = content
            && let Ok(file_tree) = gd_core::parser::parse(&content)
    ...
    if let Some(ret) = gd_class_db::method_return_type(class, method) {
...
fn format_param(p: &gd_core::gd_ast::GdParam) -> String {
    match &p.type_ann {
...
fn resolve_workspace_signature(
    ...
    for (_, content) in ws.all_files() {
        if let Ok(file_tree) = gd_core::parser::parse(&content)
