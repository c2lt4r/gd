...
pub fn query_rename(
    ...
    let edit = crate::rename::rename_cross_file(&source, &uri, position, new_name, &workspace)
    ...
    if let Ok(tree) = gd_core::parser::parse(&source) {
        ...
        let scope_names = crate::refactor::collision::collect_scope_names(
            tree.root_node(),
            &source,
            point,
            &gd_file,
        );
        if let Some(kind) = crate::refactor::collision::check_collision(new_name, &scope_names) {
...
pub fn query_rename_by_name(
    ...
    let workspace = crate::workspace::WorkspaceIndex::new(project_root.clone());
    let locations =
        crate::references::find_references_by_name(name, &workspace, file_path.as_deref(), None);
...
pub fn query_references(file: &str, line: usize, column: usize) -> Result<ReferencesOutput> {
    ...
    let locations =
        crate::references::find_references_cross_file(&source, &uri, position, true, &workspace)
            .unwrap_or_default();
...
pub fn query_references_by_name(
    ...
    let locations =
        crate::references::find_references_by_name(name, &workspace, file_path.as_deref(), class);
...
pub fn query_definition(
    ...
    line: usize,
    column: usize,
    godot: Option<&crate::godot_client::GodotClient>,
    ...
    let workspace = crate::workspace::WorkspaceIndex::new(project_root.clone());
    let response =
        crate::definition::goto_definition_cross_file(&source, &uri, position, &workspace)
...
pub fn query_hover(
    ...
    let workspace =
        gd_core::config::find_project_root(&path).map(crate::workspace::WorkspaceIndex::new);

    // Fallback: static analysis
    let content = crate::hover::hover_at(&source, position, workspace.as_ref(), Some(&path))
...
pub fn query_completions(
    ...
    if let Some(dot_items) =
        crate::completion::try_dot_completions(&source, position, workspace.as_ref())
    {
        return Ok(dot_items
            .into_iter()
    ...
    let items =
        crate::completion::provide_completions(&source, position, workspace.as_ref(), Some(&path));
...
pub fn query_scene_refs(file: &str) -> Result<Vec<SceneRefOutput>> {
    let path = resolve_file(file)?;
    let root = find_root(&path)?;
    let workspace = crate::workspace::WorkspaceIndex::new(root.clone());

    let entries = workspace.scenes_for_script(&path);
    Ok(entries
        .into_iter()
        .map(|spn| SceneRefOutput {
            scene: gd_core::fs::relative_slash(&spn.scene_path, &root),
            node: spn.node_name,
        ...
        for entry in workspace.iter_scene_connections() {
            ...
            for conn in entry.value() {
                results.push(SignalConnectionOutput {
                    ...
                    scene: gd_core::fs::relative_slash(&conn.scene_path, &root),
                });
            }
        }
    }
