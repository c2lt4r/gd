...
fn format_with_config(source: &str, config: &gd_core::config::FmtConfig) -> String {
    let tree = parser::parse(source).unwrap();
...
fn test_blank_lines_around_functions_one() {
    let config = gd_core::config::FmtConfig {
...
fn test_from_config_defaults_match_new() {
    let config = gd_core::config::FmtConfig::default();
    let input = "func a():\n\tpass\nfunc b():\n\tpass\n";
...
fn test_line_continuation_in_array() {
    ...
    let output = format_source(input);
    let tree = gd_core::parser::parse(&output).unwrap();
    assert!(
