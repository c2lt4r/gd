...
use gd_core::config::{Config, find_project_root};
...
/// Callback type for injecting extra per-file diagnostics (e.g. compiler-error checks).
pub type ExtraDiagnosticsFn =
    dyn Fn(&gd_ast::GdFile, &str, &ProjectIndex) -> Vec<LintDiagnostic> + Sync;
...
pub fn run_lint(
    paths: &[String],
    opts: &LintOptions,
    extra_diagnostics: Option<&ExtraDiagnosticsFn>,
) -> Result<()> {
    let cwd =
...
fn lint_file(
    ...
    // Include extra diagnostics (e.g. compiler-error checks) if provided
    if let Some(extra_fn) = extra_diagnostics {
        all_diags.extend(extra_fn(&file, &source, project));
    }
...
pub use gd_core::fs::{is_rule_excluded_by_override, matches_ignore_pattern};
