...
mod tests {
    ...
    // ── New: dead code after exhaustive branch doesn't cause false positive ──

    #[test]
    fn no_warning_with_dead_code_after_if_else() {
        let source =
            "func f(x) -> int:\n\tif x:\n\t\treturn 1\n\telse:\n\t\treturn 2\n\tprint(\"dead\")\n";
        assert!(check(source).is_empty());
    }
    ...
    #[test]
    fn no_warning_with_dead_code_after_match() {
        let source = "func f(x) -> String:\n\tmatch x:\n\t\t1:\n\t\t\treturn \"one\"\n\t\t_:\n\t\t\treturn \"other\"\n\tprint(\"dead\")\n";
        assert!(check(source).is_empty());
    }
}
