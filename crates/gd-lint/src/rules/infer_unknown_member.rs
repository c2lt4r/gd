use gd_core::gd_ast::{self, GdDecl, GdExpr, GdFile, GdFunc, GdStmt, GdVar};
...
fn check_inferred_var<'a>(
    ...
    if !gd_class_db::class_exists(&obj_type) {
        return;
    }

    // Check if member exists as a property, method, or signal on the resolved type
    if gd_class_db::property_exists(&obj_type, property)
        || gd_class_db::method_exists(&obj_type, property)
        || gd_class_db::signal_exists(&obj_type, property)
    {
        return;
    }
