use gd_core::gd_ast::{GdClass, GdDecl, GdFile, GdFunc};
...
fn check_inner_class(class: &GdClass, diags: &mut Vec<LintDiagnostic>) {
    let extends = match &class.extends {
        Some(gd_core::gd_ast::GdExtends::Class(c)) => *c,
        _ => return,
    };
...
fn has_warning_ignore(func: &GdFunc, warning: &str) -> bool {
    func.annotations.iter().any(|a| {
        a.name == "warning_ignore"
            && a.args.iter().any(|arg| {
                if let gd_core::gd_ast::GdExpr::StringLiteral { value, .. } = arg {
...
fn check_funcs<'a>(
    ...
    for func in funcs {
        ...
        let Some(native_sig) = gd_class_db::method_signature(extends, func.name) else {
            continue;
        };
