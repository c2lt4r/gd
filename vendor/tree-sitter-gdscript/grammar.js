...
module.exports = grammar({
  ...
  rules: {
    ...
    _identifier: ($) => /[a-zA-Z_\p{L}\p{Nl}\u3400-\u4DBF\u4E00-\u9FFF\uAC00-\uD7A3][a-zA-Z_0-9\p{L}\p{Nl}\p{Mn}\p{Mc}\p{Nd}\p{Pc}\u3400-\u4DBF\u4E00-\u9FFF\uAC00-\uD7A3]*/,
    // any "symbol"
    ...
    integer: ($) =>
      token(
        choice(
          seq(choice("0x", "0X"), repeat1(/_?[A-Fa-f0-9]+/), optional("_")),
          seq(choice("0o", "0O"), repeat1(/_?[0-7]+/), optional("_")),
          seq(choice("0b", "0B"), repeat1(/_?[0-1]+/), optional("_")),
          repeat1(/[0-9]+_?/),
        ),
      ),
    ...
    _get_node_path_segment: ($) =>
      choice(
        seq("%", choice(alias($.string, "value"), $._identifier)),
        alias($.string, "value"),
        $._identifier,
      ),
    ...
    get_node: ($) =>
      prec.right(
        seq(
          choice(
            // $path, $%unique, $/abs/path, $"string"
            seq(
              "$",
              optional(choice("/", "%")),
              choice(alias($.string, "value"), $._identifier),
              repeat(seq("/", $._get_node_path_segment)),
            ),
            // %unique, %"string", %path/to/node
            seq(
              "%",
              choice(alias($.string, "value"), $._identifier),
              repeat(seq("/", $._get_node_path_segment)),
    ...
    match_body: ($) =>
      choice(
        ...
        // GDScript allows `match expr: pass` as a no-op match statement
        seq($._indent, $.pass_statement, $._dedent),
      ),
    ...
    pair: ($) =>
      seq(
        choice(
          ...
          seq(field("left", choice($.identifier, $.string)), "="),
        ),
